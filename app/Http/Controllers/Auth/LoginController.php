...
//    --------------------end social lite
    ...
    public function redirectPath()
    {
        if (Auth::user()->role_id == 3) {
            ...
            if(Auth::user()->phone != null && Auth::user()->mobile_verified_at == null){
                return route('phone.verify', Auth::user()->id);
            }
    ...
    public function  activateWithOtp(Request $request)
    {
        ...
        {
            Toastr::error('Your verification code doesn’t matched.', 'Failed');
            return back();
    ...
    public function phoneVerifyPage($user_id)
    {
        $user = User::findOrFail($user_id);

        $page = LoginPage::getData();

        return view(theme('auth.phone_verify'), compact('user' , 'page'));
    }
        ...
        if (isModuleActive('TwoFA')) {
            ...
            if ($request->two_step_verify_code) {
                ...
                if ($requested_user->otp_code != $request->two_step_verify_code) {
                    ...
                    Toastr::error('Your verification time is expired. Please resend code for login.', 'Failed');
                    return back();
                }

                UserOtpCode::where('email', $request->email)->delete();
            } elseif ($target_user && $target_user->two_step_verification == 1) {
                ...
                if ($verification_code) {
                    send_email($user, 'Two_Factor_Authentication', [
                    ...
                    foreach (session()->get('cart') as $key => $session_cart) {
                        ...
                        if ($checkHasCourse) {
                            ...
                            if (!$enolledCousrs) {
                                ...
                                if (!$hasInCart) {
                                    $exist = Cart::where('user_id', Auth::user()->id)->latest()->frist();
                                    if ($exist) {
