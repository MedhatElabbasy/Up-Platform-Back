...
class WebsiteApiController extends Controller
{
    ...
    public function cartList()
    {
        try {
            ...
            $carts = Cart::where('user_id', Auth::id())->with('course', 'course.user', 'bundle', 'event')->get();
    ...
    public function addToCart($id)
    {
        ...
        try {
            ...
            if (Auth::check() && ($user->role_id != 1)) {
                ...
                $exist = Cart::where('user_id', $user->id)->where($field_name, $id)->first();
                ...
                if (isset($exist)) {
                    ...
                    if (isset($oldCart)) {
                        if ($type == 'course') {
                            ...
                            $message = 'Event Added to your cart';
                        } else {
                            $bundle = BundleCoursePlan::findOrFail($id);
                            $cart = new Cart();
                            $cart->user_id = $user->id;
                            ...
                            $cart->course_id = 0;
                            $cart->bundle_course_id = $id;
                            $cart->type = 'bundle';

                            $message = 'Bundle Added to your cart';
                        }
                    } else {
...
//            $quiz->duration = 0.00;
    ...
    public function finalQusSubmit(Request $request)
    {
        ...
        if (in_array('L', $request->type) || in_array('S', $request->type)) {
            ...
            $pass = 0;
            if ($score > 0) {
                $per = round($score / $totalScore * 100);
    ...
    public function news()
    {
        try {
            ...
            $query = Blog::where('status', 1)
                ->where('authored_date_time', '<', $currentTime)
                ->with('user', 'userRead');
            ...
            $blogs = $blogs->map(function ($blog) {
                $blog->image = !empty($blog->image) ? url($blog->image) : null;
                $blog->thumbnail = !empty($blog->thumbnail) ? url($blog->thumbnail) : null;
                return $blog;
            });

            $allIds = $blogs->pluck('id')->toArray();

            $read = UserBlog::whereIn('blog_id', $allIds)
    ...
    public function makeUniqueSlug($title, $existingSlugs = [])
    {
        $slug = Str::slug($title);
        $originalSlug = $slug;
        $counter = 1;
    ...
    public function storeNews(Request $request)
    {
        ...
        //   'image' => 'mimes:jpeg,jpg,png,gif,mp4,pdf|max:2048',
        ], [
            'title.required' => 'مطلوب ادخال العنوان',
            'description.required' => 'مطلوب ادخال الوصف',
            'category_id.required' => 'مطلوب ادخال الفئة',
            // 'image.mimetypes' => 'يجب أن يكون نوع الملف صورة أو فيديو أو ملف PDF.',
            // 'image.max' => 'حجم الملف يجب أن يكون أقل من 2 ميجابايت.',
        ...
        $existingSlugs = Blog::pluck('slug')->all();
        $blog->slug = $this->makeUniqueSlug($request->input('title'), $existingSlugs);

        if ($request->filled('authored_date') && $request->filled('authored_time')) {
            $blog->authored_date = $request->input('authored_date');
            $blog->authored_time = $request->input('authored_time');
        } else {

            if ($request->filled('authored_date')) {
                $blog->authored_date = $request->input('authored_date');
            } else {
                $blog->authored_date = Carbon::now()->format('m/d/Y');
            }

            if ($request->filled('authored_time')) {
                $blog->authored_time = $request->input('authored_time');
            } else {
                $blog->authored_time = Carbon::now()->startOfDay()->format('H:i');
            }
        }
        ...
        if ($request->hasFile('image')) {
            ...
            $blog->image = 'public/uploads/main/images/' . $filename;

            $blog->thumbnail = 'public/uploads/main/images/' . $filename;

            $path = 'public/uploads/main/images/' . now()->format('d-m-Y');
        ...
        return response()->json(['message' => 'تم انشاء المدونة بنجاح '], 201);
    }

    ########################

    public function displayCategoriesNews()
    {
        try {
            $blogCategories = BlogCategory::all();
            return response()->json($blogCategories, 200);
        } catch (ModelNotFoundException $e) {
            return response()->json(['error' => 'No blog categories found'], 404);
        } catch (\Exception $e) {
...
##################  display all comments on article
    public function getBlogComments($blogId)
    {
        try {
            ...
            $comments = $blog->comments()->where('status', 1)->with('user:id,name,photo,image,avatar')->get();
            $data = [
                'data' => $comments,
