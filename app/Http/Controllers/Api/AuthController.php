...
//        $credentials = request(['email', 'password']);
    ...
    public function socialLogin(Request $request)
    {
        ...
        if ($request->provider_name == 'google') {  // By Google
        ...
        } elseif ($request->provider_name == 'facebook') { // By Facebook
...
//            }
    ...
    public function getLang(Request $request)
    {
        ...
        if (auth('api')->check()) {
            $user = auth('api')->user();
            $code = empty($request->code) ? $user->language_code ?? 'ar' : $request->code;
            $rtl = empty($request->rtl) ? $user->language_rtl ?? 1 : $request->rtl;
        } else {
            $code = Settings('language_code') ?? 'ar';
            $rtl = Settings('language_rtl') ?? 1;
        }
    ...
    public function activateWithOtp(Request $request)
    {
        try {
            $status = false;
            $user = User::where('phone', $request->phone)->where('code', $request->otp)->first();
