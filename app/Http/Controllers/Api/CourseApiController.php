...
class CourseApiController extends Controller
{
    ...
    {
        $courses = Course::where('type', '1')->with('user', 'instructor', 'category', 'subCategory', 'lessons', 'lessons.quiz', 'reviews', 'comments', 'courseLevel')->where('status', 1)->where('scope', 1)->latest()->get()
            ->map(function ($i) {
                $i->points = (new CurancyExchange)->exchange($i->price);
                $i->total_lessons = count($i->lessons);
                $i->is_enrolled = $i->isLoginUserEnrolled;
                $i->course_category = 'skills_library';
    ...
    {
        $courses = Course::where('type', '3')->with('user', 'instructor', 'category', 'subCategory', 'class', 'reviews', 'comments')->where('scope', 1)->where('status', 1)->latest()->get()
            ->map(function ($i) {
                $i->points = $i->price / 2;
                $i->is_enrolled = $i->isLoginUserEnrolled;
                $i->course_category = 'online_courses';
    ...
    public function getAllCoursesQuizzes()
    {
        $courses = Course::where('quize_type', '1')
            ->where('type', '2')
            ->with('user', 'quiz')->where('scope', 1)->where('status', 1)->latest()->get();
        $response = [
            'success' => true,
            'data' => $courses,
            'total' => count($courses),
    ...
    // public function getAllMyExamsQuizzes()
    // {
    //     $courses = Course::where('quize_type', '2')
    //     ->where('type','2')
    //         ->with('user', 'quiz')->where('scope', 1)->where('status', 1)->latest()->get();
    //     $response = [
    //         'success' => true,
    ...
    // }
        ...
        {
            $courses = Course::
            // select('id', 'title', 'duration', 'image', 'price','discount_price','status','about','exam_type','certificate_name','requirements','outcomes',)
            where('type', '2')
                ->where('quize_type', '2')
            ->with(['category:id,name,image,description', 'courseLevel:id,title'])
                ->get();

            $baseURL = config('app.url');
            $courses = $courses->map(function ($course) use ($baseURL) {
                $course->image = $course->image ? $baseURL . '/' . $course->image : null;
                $course->thumbnail = $course->thumbnail ? $baseURL . '/' . $course->thumbnail : null;
                $course->trailer_link = $course->trailer_link ? $baseURL . '/' . $course->trailer_link : null;
                if ($course->category && $course->category->image) {
                    $course->category->image = $course->category->image ? $baseURL . '/' . $course->category->image : null;
                    $course->category->thumbnail = $course->category->thumbnail ? $baseURL . '/' . $course->category->thumbnail : null;
                }
                return $course;
            });
            ...
            $response = [
                ...
                'data' => $courses,
                'total' => count($courses),
                'message' => 'Getting Exam Data',
            ];
            return response()->json($response, 200);
        }

    public function getAllPassQuizzes()
    {
        $courses = Course::where('quize_type', '3')
            ->where('type', '2')
            ->with('user', 'quiz')->where('scope', 1)->where('status', 1)->latest()->get();
        $response = [
            'success' => true,
    ...
    public function getCourseDetails($id)
    {
        ...
        $course = Course::with('courseLevel', 'user', 'instructor', 'instructor.courses', 'instructor.courses.enrollUsers', 'instructor.courses.lessons', 'instructor.courses.comments', 'category', 'subCategory', 'chapters', 'chapters.lessons', 'chapters.lessons.quiz', 'lessons', 'lessons.quiz', 'reviews', 'comments', 'files')->find($id);
        ...
        if ($course) {
            ...
            $course->points = (new CurancyExchange)->exchange($course->price);
            $course->total_lessons = count($course->lessons);
            $course->is_enrolled = $course->isLoginUserEnrolled;
            $course->course_category = 'skills_library';
    ...
    {
        $relation = ['courseLevel', 'user', 'class.zoomMeetings', 'instructor', 'category'];
        ...
        $course->points = (new CurancyExchange)->exchange($course->price);
        $course->is_enrolled = $course->isLoginUserEnrolled;
        $course->course_category = 'online_courses';
    ...
    {
        $type = $request->get('course_category') == 'online_courses' ? 3 : 1;
        ...
        $query = Course::where('status', 1)->with('user')->where('type', $type);
        ...
        $courses = $query->get()
            ->map(function ($i) {
                ...
                $i->total_lessons = count($i->lessons);
                $i->is_enrolled = $i->isLoginUserEnrolled;
                $i->course_category = $i->type == 1 ? 'skills_library' : 'online_courses';
    ...
    public function subCategoriesByName($category_name)
    {
        $category_name = ucfirst(str_replace('-', ' ', $category_name));

        $category = Category::where("name->en", $category_name)->whereNull('parent_id')->firstOrFail();

        return $this->subCategories($category->id);
    }
