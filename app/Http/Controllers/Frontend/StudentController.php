...
class StudentController extends Controller
{
    ...
    public function myProfileUpdate(Request $request)
    {
        ...
        try {
            ...
            $user->language_id = (int)$lang[0] ?? 3;
            $user->language_code = $lang[1] ?? 'ar';
            $user->language_name = $lang[2] ?? 'Arabic';
            $user->language_rtl = (int)$lang[3] ?? 1;
...
//            ]);
    ...
    {
        if (!permissionCheck('student.loginActivity') && !Hash::check($request->password, auth()->user()->password)) {
