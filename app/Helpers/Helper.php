...
function convertCurrency($from_currency, $to_currency, $amount)
{
    ...
    try {
        if (Settings('currency_conversion') == 'Fixer') {
            ...
            $rate = Cache::remember('CurrencyRateFixer' . $to . SaasDomain(), now()->addDay(), function () use ($from, $to, $amount, $client) {
            ...
            $rate = Cache::remember('CurrencyRateExchangerate' . $to . SaasDomain(), now()->addDay(), function () use ($from, $to, $amount, $client) {
...
if (!function_exists('paymentGateWayCredentialsEmptyCheck')) {
    ...
    {
        if ($method == 'Tap') {
            if (!empty(getPaymentEnv('Tap_API_KEY')) && !empty(getPaymentEnv('Tap_API_SECRET')) && !empty(getPaymentEnv('IS_TAP_LOCALHOST')) 
            && !empty(getPaymentEnv('Tap_success_url')) && !empty(getPaymentEnv('Tap_fail_url'))) {
                $result = true;
            } else {
                $result = false;
            }
...
if (!function_exists('orgGetStartEndDate')) {
    function orgGetStartEndDate($enroll, $course)
    {
        ...
        if ($enroll->org_subscription_plan_id != 0) {
            ...
            if ($shift) {
                $days['start'] = $shift->start_date . ' ' . $shift->start_time;
                $days['end'] = $shift->end_date . ' ' . $shift->end_time;
            } else {
...
//            $point = 0;
            ...
            \Modules\Org\Entities\OrgLeaderboardUserPoint::create([
                'user_id' => Auth::id(),
                'type' => $type,
                'point' => (int)$point,
