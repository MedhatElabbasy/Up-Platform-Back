...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function sendSMSVerificationNotification()
    {
        if (!Session::has('reg_phone')) {
            try {
                Session::put('reg_phone', $this->phone);
                $this->update(['code' => mt_rand(1111, 9999), 'code_expire' => Carbon::now()->addMinutes(5)]);
                $this->sendSms($this->phone , 'Your verification code is ' . $this->code);
    ...
    public function lotteryWheel()
    {
        return $this->belongsTo(LotteryWheel::class);
    }
    public function clubEvents()
    {
        return $this->belongsToMany(ClubEvent::class, 'club_event_users');
    }

    public function consultantEducations()
    {
        return $this->hasOne(ConsultantEducation::class, 'user_id');
    }

    public function ConsultantPackageDetail()
    {
