            ...
            <div class="row">
                            ...
                            @if(isset($edit))
                                @if (permissionCheck('course.category.store'))
                                    <a href="{{route('course.category')}}"
                                       ...
                                       title="{{__('courses.Add New')}}">+ {{__('courses.Add New')}}</a>
                                @endif
                            @endif
                        </div>
                    </div>
                                        ...
                                        <form action="{{route('course.category.store') }}" method="POST"
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-xl-12">
                                                    <div class="primary_input mb-25">
                                                        <label class="primary_input_label"
                                                               for="parent">{{ 'الفئة' }}</label>
                                                        <select class="primary_select mb-25" name="parent"
                                                                id="parent">
                                                            <!--<option value="">{{__('common.None')}}</option>-->
                                                            @foreach($categories_box as $category)
                                                                ...
                                                                <option
                                                                    value="{{$category->id}}"
                                                                    @selected((ucfirst(str_replace('-', ' ', $category_name)))== ($category->getTranslations('name')['en']))
                                                ...
                                                <!--<div class="col-lg-12 mt-10">
                                                    <div class="primary_input mb-15">
                                                        ...
                                                        <div class="primary_file_uploader">
                                                            <input class="primary-input filePlaceholder courseLabel"
                                                                   type="text"
                                                            ...
                                                            <div class="multi_btn">
                                                                <button class="" type="button">
                                                                    ...
                                                                    <input type="file"
                                                                           class="d-none fileUpload courseLabel"
                                                                           name="photo"
                                                                           id="document_file_1">
                                                                </button>
                ...
                <div class="col-lg-9">
                    <div class="box_header common_table_header">
                        ...
                        @if(!isset($edit))
                        <select class="primary-btn white p-2" onchange="window.location.href = this.value">
                            <option value="{{route('course.category')}}" @selected(!$category_name)>{{ 'الكل' }}</option>
                            <option value="{{route('course.category', 'skills-library')}}" @selected($category_name && $category_name=='skills-library')>{{ 'مكتبة المهارات' }}</option>
                            <option value="{{route('course.category', 'online-courses')}}" @selected($category_name && $category_name=='online-courses')>{{ 'الدورات الاونلاين' }}</option>
                        </select>
                        @else
                        <select class="primary-btn white p-2" onchange="window.location.href = this.value">
                            <option value="{{route('course.category.edit', $edit->id)}}" @selected(!$category_name)>{{ 'الكل' }}</option>
                            <option value="{{route('course.category.edit', ['id'=>$edit->id, 'category_name'=>'skills-library'])}}" @selected($category_name && $category_name=='skills-library')>{{ 'مكتبة المهارات' }}</option>
                            <option value="{{route('course.category.edit', ['id'=>$edit->id, 'category_name'=>'online-courses'])}}" @selected($category_name && $category_name=='online-courses')>{{ 'الدورات الاونلاين' }}</option>
                        </select>
                        @endif
                    </div>
                                            ...
                                            <td>
                                                ...
                                                <div class="dropdown CRM_dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right"
                                                        ...
                                                        @if (permissionCheck('course.category.edit') && orgPermission($category->organization_id))
                                                            <a class="dropdown-item edit_brand"
                                                            
                                                               href="{{route('course.category.edit', ['id'=>$category->id, 'category_name'=>$category_name] )}}">{{__('common.Edit')}}</a>
                                                        @endif
                                                        @if (permissionCheck('course.category.delete') && orgPermission($category->organization_id))
