    ...
    <div class="col-lg-12 card p-4">
        ...
        <form method="POST" action="{{ route('dashboard.categories.update', ['category'=>$category->id]) }}">
            ...
            <div class="form-group">
                ...
                <input name="name" type="text" class="form-control" placeholder="ادخل الاسم" value="{{$category->name}}" required>
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">الوصف</label>
                <textarea name="description" class="form-control" rows="5" placeholder="ادخل الوصف" required>{{$category->description}}</textarea>
            </div>
