...
use App\Http\Controllers\Admin\ChangeCapController;
use App\Http\Controllers\Club\CLubEventListController;
use App\Http\Controllers\Club\CLubEventUsersListController;
use App\Http\Controllers\Prize\WhellPrizeController;
...
Route::group(['namespace' => 'Api', 'prefix' => 'api'], function () {
    // Redirect to website
    Route::get('redirect-to-website', 'RedirectController@redirectToWebsite');
    Route::get('redirect-to-continue-course', 'RedirectController@redirectToCourse');
});

//change curances
Route::get('change_cap', [ChangeCapController::class, 'index'])->name('changeCap');
//club
Route::group(['prefix' => 'club-events', 'middleware' => ['auth', 'admin']], function () {
    Route::get('/', [CLubEventListController::class, 'index'])->name('club_events.index');
    Route::get('/create', [CLubEventListController::class, 'create'])->name('club_events.create');
    Route::post('/store', [CLubEventListController::class, 'store'])->name('club_events.store');
    Route::get('/edit/{id}', [CLubEventListController::class, 'edit'])->name('club_events.edit');
    Route::put('/{id}', [CLubEventListController::class, 'update'])->name('club_events.update');
    Route::delete('/destroy/{club_event}', [CLubEventListController::class, 'destroy'])->name('club_events.destroy');
    Route::prefix('/users')->group(function () {
        Route::get('/', [CLubEventUsersListController::class, 'index'])->name('club_events.user.index');
        Route::delete('{eventId}/{userId}', [CLubEventUsersListController::class, 'destroy'])->name('events.users.destroy');
    });
});
########           Community  ### finance
//
Route::group(['prefix' => 'finance', 'middleware' => ['auth', 'admin']], function () {
    Route::resource('funding-agencies', FundingAgencyController::class);
    Route::resource('fund-categories', FundCategoriesController::class);

});

   ########           Community  ### opportunities
Route::group(['prefix' => 'opportunities', 'middleware' => ['auth', 'admin']], function () {
Route::resource('opportunities', OpportunityWebController::class);
Route::resource('opportunity-categories', OpportunityCategoryWebController::class);

});

   ########           Community  ### partnership
   Route::group(['prefix' => 'partnership', 'middleware' => ['auth', 'admin']], function () {
    Route::resource('partnerships', PartnershipWebController::class);
   });




Route::prefix('prizes/wheel')->group(function () {
    Route::get('/', [WhellPrizeController::class, 'index'])->name('whell.index_prize');
    Route::get('/create', [WhellPrizeController::class, 'create'])->name('whell.create_prize');
    Route::post('/store', [WhellPrizeController::class, 'store'])->name('whell.store_prize');
    Route::get('/edit/{id}', [WhellPrizeController::class, 'edit'])->name('whell.edit_prize');
    Route::put('/update/{id}', [WhellPrizeController::class, 'update'])->name('whell.update_prize');
    Route::delete('/destroy/{id}', [WhellPrizeController::class, 'destroy'])->name('whell.destroy_prize');
});
...
Route::get('phone-verify/{user_id}', '\App\Http\Controllers\Auth\LoginController@phoneVerifyPage')->name('phone.verify');
Route::post('activate-phone', '\App\Http\Controllers\Auth\LoginController@activateWithOtp')->name('activateWithOtp');
...
Route::group(['namespace' => 'Frontend'], function () {
    ...
    // consultants-frontend
    Route::get('consultants', 'ConsultantsController@consultants')->name('consultants');
    Route::get('consultantDetails/{id}/{name}', 'ConsultantsController@consultantDetails')->name('consultantDetails');
    Route::post('/consultants/{id}/ask-for-advice', 'ConsultantsController@askForAdvicemsg')->name('consultant.ask-for-advice');
    Route::post('/consultants/{id}/ask-for-appointment', 'ConsultantsController@askForAppointment')->name('consultant.ask-for-appointment');
        Route::get('/get-appointment-times/{id}', 'ConsultantsController@getAppointmentTimes')->name('get-appointment-times');
...
Route::group(['prefix' => 'order', 'middleware' => ['auth']], function () {
    ...
    Route::get('paypal/failed', 'PaymentController@paypalFailed')->name('paypalFailed');
      ...
      Route::get('tap/success', 'PaymentController@tapSuccess')->name('tapSuccess');
   Route::get('tap/failed', 'PaymentController@tapFailed')->name('tapFailed');
});
//deposit
Route::group(['prefix' => 'deposit', 'middleware' => ['auth']], function () {
    ...
    Route::get('tapDepositeSuccess', 'DepositController@tapDepositeSuccess')->name('tapDepositeSuccess');
    Route::get('tapDepositeFailed', 'DepositController@tapDepositeFailed')->name('tapDepositeFailed');
...
Route::group(['middleware' => ['auth']], function () {
    ...
    #####Cap
    Route::get('cap-price', 'CapController@index')->name('CapPrice');
    //
    Route::post('cap-price/store', 'CapController@store')->name('CapPrice.store');
    #####
...
Route::group(['namespace' => 'Admin', 'prefix' => 'course', 'as' => 'course.', 'middleware' => ['auth', 'admin']], function () {
    ...
    Route::get('categories/edit/{id}/{category_name?}', 'CourseController@category_edit')->name('category.edit')->middleware('RoutePermissionCheck:course.category.edit');
    ...
    Route::get('categories/{category_name?}', 'CourseController@category')->name('category')->middleware('RoutePermissionCheck:course.category');
...
// Suggested Path Test
Route::group(['namespace' => 'SuggestedPathTest', 'as' => 'suggested-path-test.', 'prefix' => 'admin/suggested-path-test'], function () {
    Route::get('settings', 'SuggestedPathTestController@settings')->name('settings.index');
    Route::post('settings', 'SuggestedPathTestController@settings_update')->name('settings.store');
    Route::get('/', 'SuggestedPathTestController@index')->name('index');
    Route::post('/', 'SuggestedPathTestController@store')->name('store');
    Route::post('/delete', 'SuggestedPathTestController@delete')->name('delete');
    Route::get('/edit/{id}', 'SuggestedPathTestController@edit')->name('edit');
    Route::post('/edit/{id}', 'SuggestedPathTestController@update')->name('update');

    Route::post('/choices/delete', 'SuggestedPathTestChoiceController@delete')->name('choices.delete');
    Route::get('/choices/edit/{id}', 'SuggestedPathTestChoiceController@edit')->name('choices.edit');
    Route::post('/choices/edit/{id}', 'SuggestedPathTestChoiceController@update')->name('choices.update');
    Route::get('/choices/{question_id}', 'SuggestedPathTestChoiceController@index')->name('choices.index');
    Route::post('/choices/{question_id}', 'SuggestedPathTestChoiceController@store')->name('choices.store');
});

// Simulation
Route::group(['namespace' => 'Simulation', 'as' => 'simulation.', 'prefix' => 'admin/simulation'], function () {
    Route::group(['as' => 'categories.', 'prefix' => 'categories'], function () {
        Route::get('/', 'SimulationCategoryController@index')->name('index');
        Route::post('/', 'SimulationCategoryController@store')->name('store');
        Route::post('/delete', 'SimulationCategoryController@delete')->name('delete');
        Route::get('/edit/{id}', 'SimulationCategoryController@edit')->name('edit');
        Route::post('/edit/{id}', 'SimulationCategoryController@update')->name('update');
    });

    Route::group(['as' => 'lessons.', 'prefix' => 'lessons'], function () {
        Route::get('/', 'SimulationCategoryController@index')->name('index');
        Route::post('/', 'SimulationCategoryController@store')->name('store');
        Route::post('/delete', 'SimulationCategoryController@delete')->name('delete');
        Route::get('/edit/{id}', 'SimulationCategoryController@edit')->name('edit');
