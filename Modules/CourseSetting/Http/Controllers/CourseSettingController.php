...
class CourseSettingController extends Controller
{
    ...
    public function getAllCourse(Request $request)
    {
        try {
            ...
            $parent_category = Category::where('name->en', 'Skills library')->firstOrFail();

            $query = Category::where('parent_id', $parent_category->id)->where('status', 1)->orderBy('position_order', 'ASC');
    ...
    public function saveCourse(Request $request)
    {
        ...
        try {
            ...
            if($request->has('quize_type')){
                $course->quize_type = $request->quize_type;
            }
