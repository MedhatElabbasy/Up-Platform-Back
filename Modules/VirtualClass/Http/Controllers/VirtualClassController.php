...
class VirtualClassController extends Controller
{
    ...
    {
        $parent_category = Category::where("name->en", 'Online courses')->first()->id;
        ...
        $data = [
            ...
            'classes' => $classes,
            'categories' => Category::with('childs')->whereNotNull('parent_id')->where('parent_id', $parent_category)->where('status', 1)->orderBy('position_order', 'ASC')->get(),
        ];
    ...
    public function edit($id)
    {
        ...
        $data = [
            ...
            'categories' => Category::whereNotNull('parent_id')->where('parent_id', $parent_category)->where('status', 1)->orderBy('position_order', 'ASC')->get(),
        ];
        if (isModuleActive('CertificatePro') && Settings('use_certificate_template') == 'pro') {
            if (Auth::user()->role_id == 1) {
                $certificates_query = CertificateTemplate::query();
