...
class PaymentMethodSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($method->method == 'Tap') {

                try {
                    $method_setup = PaymentMethodCredential::firstOrNew(array('lms_id' => $method->lms_id));

                    $method_setup->Tap_API_KEY = $request->Tap_API_KEY;
                    $method_setup->Tap_API_SECRET = $request->Tap_API_SECRET;
                    $method_setup->Tap_URL = $request->Tap_URL;
                    
                    $method_setup->Tap_success_url = $request->Tap_success_url;
                    $method_setup->Tap_fail_url = $request->Tap_fail_url;

                    if ($request->IS_TAP_LOCALHOST == 2) {
                        $value3 = false;
                    } else {
                        $value3 = true;
                    }
                    $method_setup->IS_TAP_LOCALHOST = $value3;
            ...
            }elseif ($method->method == 'Sslcommerz') {
    ...
    public function testSubmit(Request $request)
    {
        ...
        if ($data['method'] == "Sslcommerz") {
        ...
        } elseif ($data['method'] == 'Tap') {


            $redirect_url  = getPaymentEnv('Tap_success_url');
            $curl = curl_init();
            ...
            curl_setopt_array($curl, array(
                ...
                CURLOPT_POSTFIELDS => "{\"amount\":$request->test_amount,\"currency\":\"SAR\",\"threeDSecure\":true,\"save_card\":false,\"description\":\"description\",\"statement_descriptor\":\"Sample\",\"metadata\":{\"order_id\":$request->order_id,\"subscription_id\":\"\",\"user_id\":$request->user_id},\"reference\":{\"transaction\":\"txn_0001\",\"order\":\"$request->order_id\"},\"receipt\":{\"email\":false,\"sms\":true},\"customer\":{\"first_name\":\"zahr\",\"middle_name\":\"''\",\"last_name\":\"''\",\"email\":\"zahr@gmail.com\",\"phone\":{\"country_code\":\"\",\"number\":\"\"}},\"source\":{\"id\":\"src_card\"},\"post\":{\"url\":\"https://admin.up.com/post_url\"},\"redirect\":{\"url\":\"$redirect_url?lang=$request->lang&user_id=$request->user_id&price=$request->test_amount&order_id=$request->order_id\"}}",
    ...
    //tap payment with success return 
    public function Tap_success()
    {
        Toastr::success('Payment successfull', 'Success');
        return redirect()->route('paymentmethodsetting.test');
    } 
    ...
    //tap fail
    public function Tap_fail()
    {
        Toastr::error('Fail payment.', 'Failed');
        return redirect()->route('paymentmethodsetting.test');
    }
